...
_token_name_can_device = "driver device"
_token_name_can_channel = "driver device channel"
...
def c_open_channel(device_handle, channel_number, bps, work_mode, acc_code, acc_mask):
    ...
    if status != 1:
        log_print("set bps to", bps, "failed!")
        return 0
    else:
        log_print("set bps to", bps, "successed!")

    status = _zlg_dll.VCI_InitCAN(devtype, devidx, channel_number, pointer(ic))
    if status != 1:
        log_print("configure failed, dev, dev-idx, channel-idx", devtype, devidx, acc_mask)
        return 0

    status = _zlg_dll.VCI_StartCAN(devtype, devidx, channel_number)
    if status != 1:
        log_print("start channel", channel_number, "failed!")
        return 0
    else:
        log_print("start channel", channel_number, "successed!")
