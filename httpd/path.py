...
class PathProcessorWrapper:
    def __init__(self, processor, origin_express, app, match_express, match_dmap, methods, **kwargs):
    ...
    def match(self, path, method):
        ...
        return params_dict, self.wrapper_process, self.kwargs
...
def bind(origin_path_express, processor, app=None, method=None, **kwargs):
    if app is None:
        app = 'main'
    ...
    return PathProcessorWrapper(processor, origin_path_express, app, match_express, match_dmap, method, **kwargs)
...
def search_route_path(request):
    ...
    for router in _global_path_router:
        params_dict, processor, kwargs = router.match(request.path, request.method)
        if params_dict is None:
            continue

        return params_dict, processor, kwargs
    return dict(), default_file_handle, dict()
